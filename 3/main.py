...
def calculate_entropy(text):
    frequencies = Counter(text)  # словарь каждого символа в тексте содержит частоту
...
def huffman_coding(frequencies):
    ...
    heapq.heapify(heap)  # минимальная куча
    while len(heap) > 1:
        lo = heapq.heappop(heap)  # извлекаем наименьший элемент
        hi = heapq.heappop(heap)  # извлекаем следующий наименьший элемент
        for pair in lo[1:]:
            pair[1] = '0' + pair[1]  # левая ветвь с 0
        for pair in hi[1:]:
            pair[1] = '1' + pair[1]  # правая ветвь с 1
...
def calculate_code_length_and_efficiency(codes, frequencies):
    total_length = sum(len(code) * frequencies[symbol] for symbol, code in
                       codes)  # длина закодированного текста, которая учитывает частоту каждого символа
...
def encode_text(text, encoding_dict):
    return ''.join(encoding_dict[symbol] for symbol in text)  # возвращается закодированная строчка
...
processed_text = text.lower()  # приведем текст к нижнему регистру
...
# Задача 4
# Повторите аналогичные шаги для двухбуквенных сочетаний
frequencies = Counter(processed_text)
huffman_codes = huffman_coding(frequencies)
average_length, efficiency = calculate_code_length_and_efficiency(huffman_codes, frequencies)
